import { getRole, route } from "../_lib/http.js";
...
export default route(["PUT", "DELETE"], async (req, res) => {
  ...
  if (req.method === "DELETE") {
    await removePosition(scope, code);
    const snapshot = await refreshSnapshot(scope);
  ...
  const existing = (await loadPositions(scope)).find((p) => p.code === code);
  ...
  await upsertPosition(scope, position);
  const snapshot = await refreshSnapshot(scope);
  res.json(toResponse(snapshot, false));
});
